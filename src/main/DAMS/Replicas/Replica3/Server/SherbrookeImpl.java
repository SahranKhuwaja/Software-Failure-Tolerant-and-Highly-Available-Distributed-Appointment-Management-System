...
public class SherbrookeImpl implements HospitalInterface{


    HashMap<String, HashMap<String,Integer>> SHEA =new HashMap<String, HashMap<String,Integer>>();

    HashMap<String,ArrayList<String>> SHEP =new HashMap<String,ArrayList<String>>();

    DateFormat df = new SimpleDateFormat("yyyy/MM/dd HH:mm:ss");
    Date d = new Date();


    public SherbrookeImpl(){
        super();
        ...
        al1.add("SHEA210222 S");
        al1.add("SHEA200222 P");
        SHEP.put("SHEP1234", al1);

        ArrayList<String> al2 = new ArrayList<String>();
        al2.add("SHEA220222 D");
        al2.add("SHEA200222 P");
        SHEP.put("MTLP1234", al2);

        ArrayList<String> al3 = new ArrayList<String>();
        al3.add("SHEA220222 D");
        al3.add("SHEA200222 P");
        SHEP.put("QUEP1234", al3);
    ...
    public String localCheckAvailability(String appointmentID, String appointmentType) {
        ...
        boolean has_type= SHEA.containsKey(appointmentType);
        boolean has_id= SHEA.get(appointmentType).containsKey(appointmentID);
        int c=0;
        if(has_id && has_type) {
            c = SHEA.get(appointmentType).get(appointmentID);
        }
    ...
    public synchronized String swapAppointment(String patientID, String oldAppointmentID, String oldAppointmentType,
        ...
        // Check
        if(SHEP.get(patientID)!=null) {
            lst = SHEP.get(patientID);
            ...
            switch(city) {
                case "MTL": {
                    ...
                    if(msg2.equals("Yes")) {
                        ...
                        app2 = SHEP.get(patientID);
                        app2.add(newAppointmentID+" "+newAppointmentType);
                        SHEP.put(patientID, app2);
                    }else {
                ...
                case "SHE":{msg2  = localBook(patientID, newAppointmentID, newAppointmentType);}
            }
        }catch (Exception e) {
            msg2 = "err";
        }
    ...
    public synchronized String addAppointment(String appointmentID, String appointmentType, Integer capacity) {
        ...
        try {
            ...
            output.append(df.format(d)+"Request for adding Appointmetn Received at Sherbrooke Server"+System.lineSeparator());
            ...
            SHEA.get(appointmentType).put(appointmentID, capacity);
    ...
    public synchronized String removeAppointment(String appointmentID, String appointmentType) {
        BufferedWriter output = logWriter();
        try {
            output.append(df.format(d)+"Request for Remove Appointment Received at Sherbrooke Server"+System.lineSeparator());
            ...
            if(SHEA.get(appointmentType).get(appointmentID) != null){
                SHEA.get(appointmentType).remove(appointmentID);
                ...
                Set<String> keys = SHEP.keySet();
                for(String s : keys) {
                    if(SHEP.get(s).contains(PappointmentId)) {
        ...
        try {
            ...
            switch(city) {
                case "MTL": {
                    ...
                    SHEP.get(patientId).remove(appointmentId);
    ...
    public String listAppointmentAvailability(String appointmentType) {
        ...
        try {
            output.append(df.format(d)+"Request for View Appointment Details Received at Sherbrooke Server"+System.lineSeparator());
            ...
            output.append(df.format(d)+"Appointment Details from Sherbrooke Server"+msg1+System.lineSeparator());

        } catch (IOException e) {
            msg = "err";
            System.out.println("error in Sherbrooke in list availability");
        }


        try {
            output.append(df.format(d)+" Calling Montreal server from Sherbooke Server for Appointment Details "+System.lineSeparator());
            ...
            msg = "err";
            System.out.println("error while calling the Montreal from Sherbrooke in list availability");
        }


        try {
            output.append(df.format(d)+" Calling Quebec server from Sherbrooke Server for Appointment Details "+System.lineSeparator());
            msg3 = InterServerUDPSender.listAppointment(3131, appointmentType);
            output.append(df.format(d)+" Appointment Details from Quebec Server "+msg3+System.lineSeparator());
        } catch (IOException e) {
            msg = "err";
            System.out.println("error while calling the quebec from Sherbrooke in list availability");
        }
    ...
    public String localBook(String patientID, String  appointmentID, String appointmentType) {
        ...
        try {
            ...
            if(has_type && has_id && c>0) {
                ...
                if(SHEP.containsKey(patientID)) {
                    if(SHEP.get(patientID).contains(PappointmentID)) {
                        // Appointment Already exist
                        msg = "No";

                    }else {
                        l = SHEP.get(patientID);
                        l.add(PappointmentID);
                        SHEP.put(patientID,l);
                        c = c-1;
                        SHEA.get(appointmentType).put(appointmentID,c);
                        msg = "Yes";
                    }
    ...
    public synchronized String bookAppointment(String patientID, String appointmentID, String appointmentType) {
        ...
        try {
            output.append(df.format(d)+" Request for Book Appointment Received at Sherbrooke Server "+System.lineSeparator());
            ...
            switch(city) {
                ...
                case "SHE":{
                    output.append(df.format(d)+" Book Appointment at Sherbrooke Server "+System.lineSeparator());
    ...
    public String getAppointmentSchedule(String patientID) {
        ...
        try {
            output.append(df.format(d)+" Request for Get Appointment Schedule Received at Sherbrooke Server "+System.lineSeparator());
            output.append(df.format(d)+" Data From Client"+" Patient ID "+patientID+System.lineSeparator());
            boolean has_type=SHEP.containsKey(patientID);
            if(has_type) {
                l = SHEP.get(patientID);
                int size = l.size();
                if(size>0) {
        ...
        try {
            if(SHEP.get(patientID)!=null) {
                ...
                if(lst.contains(PappointmentID)) {
                    ...
                    SHEP.put(patientID,lst);

                    int count = SHEA.get(appointmentType).get(appointmentID);
                    count++;
                    SHEA.get(appointmentType).put(appointmentID, count);
                    msg = "Yes";
    ...
    public synchronized String cancelAppointment(String patientID, String appointmentID, String appointmentType) {
        ...
        try {
            output.append(df.format(d)+" Request for Cancel Appointment Received at Sherbrooke Server "+System.lineSeparator());
            output.append(df.format(d)+" Data From Client"+" Patient ID "+patientID+" AppointmentID "+appointmentID+" Appointment Type "+appointmentType+System.lineSeparator());
            ...
            switch(city) {
                ...
                case "SHE":{
                    output.append(df.format(d)+" Cancel Sherbrooke Server Appointment "+System.lineSeparator());
        ...
        public void run(){
            ...
            try {
                ...
                while (true) {
                    ...
                    try {
                        output.append(df.format(d)+" Inter Server Request Received at Sherbrooke Server "+System.lineSeparator());
                        ...
                        System.out.println("Error from SHE server UDP innerloop"+ e);
                    }
                }

            } catch (Exception e) {
                System.out.println("Error from SHE server UDP"+ e);
            }
